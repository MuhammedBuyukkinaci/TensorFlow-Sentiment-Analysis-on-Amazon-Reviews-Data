...
nodes_on_FC = 16 # Number of nodes on FC layer
epsilon = 1e-4# For batch normalization
val_loop_iter = 50 # in how many iters we record
filter_width = 3 # filter width in tf.nn.conv2d
...
#Adding a 4th dimension to implement CONV2D
data_reshaped = tf.expand_dims(data, 3)

print(data_reshaped.get_shape().as_list())

###########MAX POOLING######################

##CONVOLUTION LAYER 1 -CONV2D
#Weights for layer 1
w_1_mp = tf.Variable(tf.truncated_normal([filter_width, filter_width, 1, 16], stddev=0.01))
#Bias for layer 1
b_1_mp = tf.Variable(tf.constant(0.0, shape=[[filter_width, filter_width, 1, 16][3]]))
#Applying convolution
c_1_mp = tf.nn.conv2d(data_reshaped, w_1_mp,strides=[1, 1, 1, 1], padding='SAME')
#Adding bias
c_1_conv2d_without_activation_mp = c_1_mp + b_1_mp
#Applying RELU
data_conv2d_1_without_pooling_mp = tf.nn.relu(c_1_conv2d_without_activation_mp)

print(data_conv2d_1_without_pooling_mp.get_shape().as_list())

##POOLING LAYER1
data_conv2d_1_mp = tf.nn.max_pool(data_conv2d_1_without_pooling_mp, ksize=[1, 3, 3, 1],strides=[1, 2, 2, 1], padding='SAME')
print(data_conv2d_1_mp.get_shape().as_list())

##CONVOLUTION LAYER 2 -CONV2D
#Weights for layer 2
w_2_mp = tf.Variable(tf.truncated_normal([filter_width, filter_width, 16, 32], stddev=0.01))
#Bias for layer 2
b_2_mp = tf.Variable(tf.constant(0.0, shape=[[filter_width, filter_width, 16, 32][3]]))
#Applying convolution
c_2_mp = tf.nn.conv2d(data_conv2d_1_mp, w_2_mp,strides=[1, 1, 1, 1], padding='SAME')
#Adding bias
c_2_conv2d_without_activation_mp = c_2_mp + b_2_mp
#Applying RELU
data_conv2d_2_without_pooling_mp = tf.nn.relu(c_2_conv2d_without_activation_mp)

print(data_conv2d_2_without_pooling_mp.get_shape().as_list())

##POOLING LAYER2
data_conv2d_2_mp = tf.nn.max_pool(data_conv2d_2_without_pooling_mp, ksize=[1, 3, 3, 1],strides=[1, 2, 2, 1], padding='SAME')
print(data_conv2d_2_mp.get_shape().as_list())

#Flattening data_conv2d_2_mp
flattened_mp = tf.reshape(data_conv2d_2_mp,[-1,shape_[1]*shape_[2]*shape_[3]])
print(flattened_mp.get_shape().as_list())


########AVG POOLING#############
##CONVOLUTION LAYER 1 -CONV2D
#Weights for layer 1
w_1_ap = tf.Variable(tf.truncated_normal([filter_width, filter_width, 1, 16], stddev=0.01))
#Bias for layer 1
b_1_ap = tf.Variable(tf.constant(0.0, shape=[[filter_width, filter_width, 1, 16][3]]))
#Applying convolution
c_1_ap = tf.nn.conv2d(data_reshaped, w_1_ap,strides=[1, 1, 1, 1], padding='SAME')
#Adding bias
c_1_conv2d_without_activation_ap = c_1_ap + b_1_ap
#Applying RELU
data_conv2d_1_without_pooling_ap = tf.nn.relu(c_1_conv2d_without_activation_ap)

print(data_conv2d_1_without_pooling_ap.get_shape().as_list())

##POOLING LAYER1
data_conv2d_1_ap = tf.nn.avg_pool(data_conv2d_1_without_pooling_ap, ksize=[1, 3, 3, 1],strides=[1, 2, 2, 1], padding='SAME')
print(data_conv2d_1_ap.get_shape().as_list())

##CONVOLUTION LAYER 2 -CONV2D
#Weights for layer 2
w_2_ap = tf.Variable(tf.truncated_normal([filter_width, filter_width, 16, 32], stddev=0.01))
#Bias for layer 2
b_2_ap = tf.Variable(tf.constant(0.0, shape=[[filter_width, filter_width, 16, 32][3]]))
#Applying convolution
c_2_ap = tf.nn.conv2d(data_conv2d_1_ap, w_2_ap,strides=[1, 1, 1, 1], padding='SAME')
#Adding bias
c_2_conv2d_without_activation_ap = c_2_ap + b_2_ap
#Applying RELU
data_conv2d_2_without_pooling_ap = tf.nn.relu(c_2_conv2d_without_activation_ap)

print(data_conv2d_2_without_pooling_ap.get_shape().as_list())

##POOLING LAYER2
data_conv2d_2_ap = tf.nn.avg_pool(data_conv2d_2_without_pooling_ap, ksize=[1, 3, 3, 1],strides=[1, 2, 2, 1], padding='SAME')
print(data_conv2d_2_ap.get_shape().as_list())

#Flattening data_conv2d_2_mp
flattened_ap = tf.reshape(data_conv2d_2_ap,[-1,shape_[1]*shape_[2]*shape_[3]])
print(flattened_ap.get_shape().as_list())

#Concating 2 different pooled & flattened versions
flattened = tf.concat([flattened_mp,flattened_ap],axis=1)
print(flattened.get_shape().as_list())

shape_ = flattened.get_shape().as_list()

#Defining weights and biases for 1 st Fully Connected part of NN
weight_fc1 = tf.Variable(tf.truncated_normal([shape_[1], nodes_on_FC]))
bias_fc1 = tf.Variable(tf.constant(0.1, shape=[nodes_on_FC]))

#Defining 1st FC layer
y_pred_without_BN = tf.matmul(flattened, weight_fc1) + bias_fc1
